...
uint8_t eth_type_is_arp_and_my_ip(uint8_t* buf, unsigned  int len)
{
   uint8_t i = 0;
   //  
   if (len < 41)
...
uint8_t eth_type_is_ip_and_my_ip(uint8_t* buf, unsigned  int len)
{
   uint8_t i = 0;
...
void make_echo_reply_from_request(uint8_t* buf, unsigned  int len)
{
   make_eth(buf);
   make_ip(buf);
...
void make_udp_reply_from_request(uint8_t* buf, char* data, uint8_t datalen, unsigned  int port)
{
   uint8_t i = 0;
   unsigned  int ck;
   make_eth(buf);
...
unsigned  int fill_tcp_data_p(uint8_t* buf, unsigned  int pos,
...
// tcp data. Returns the position at which the string after
// this string could be filled.
unsigned  int fill_tcp_data(uint8_t* buf, unsigned  int pos,
   const char* s)
{
   // fill in tcp data at position pos
   //
...
// length and checksum
void make_tcp_ack_with_data(uint8_t* buf, unsigned  int dlen)
{
   unsigned  int j;
   // fill the header:
